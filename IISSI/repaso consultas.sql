...
*/
SELECT u.nombre, e.salario FROM empleados e JOIN usuarios u ON e.usuarioId=u.id;

/*
Obtén los productos de la tabla `Productos` que tienen un precio mayor o igual a 20.00.
*/
SELECT p.nombre FROM productos p WHERE p.precio>= 20;


/*
Lista los clientes con su dirección de envío, código postal y fecha de nacimiento.
*/
SELECT direccionEnvio, codigoPostal, fechaNacimiento FROM clientes;
...
*/
SELECT AVG(e.salario) AS salario_promedio FROM empleados e;

/*
Obtén el número total de productos agrupados por tipo de producto.
*/
SELECT tp.nombre, COUNT(pr.id) AS numero_productos FROM tiposproducto tp JOIN productos pr ON tp.id=pr.tipoProductoId GROUP BY tp.nombre;


/*
Lista los pedidos realizados por cada cliente con su nombre y la fecha de realización.
*/
SELECT u.nombre, p.fechaRealizacion, p.id
FROM usuarios u
...
*/
SELECT u.nombre AS nombre_cliente, COUNT(DISTINCT lp.productoId) AS numero_productos_distintos
FROM usuarios u
JOIN clientes c ON u.id=c.usuarioId
